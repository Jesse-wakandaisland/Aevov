...
class APSIntegration {
    ...
    private function handle_large_pattern($pattern_data) {
        ...
        for ($i = 0; $i * $chunk_size < $total_size; $i++) {
            $chunk_data = substr($tensor_bytes, $i * $chunk_size, $chunk_size);
            $chunks[] = [
                'sku' => $this->generate_sku(),
                'is_partial' => true,
                'part' => $i,
                'total_parts' => ceil($total_size / $chunk_size),
                'data' => base64_encode($chunk_data),
                'original_pattern_id' => $pattern_data['id']
            ];
        }
