...
register_activation_hook(__FILE__, function() {
    if (!class_exists('APSTools') && !class_exists('APSTools\APSTools') && !class_exists('APS\APS_Plugin')) {
...
class BloomChunkScanner {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_start_chunk_scan', function() {
            ...
            error_log('start_chunk_scan request data: ' . print_r($_POST, true));
            $this->ajax_start_scan();
        });

        add_action('wp_ajax_get_scan_progress', function() {
            check_ajax_referer('bloom-scanner-nonce', 'nonce');
            error_log('get_scan_progress request data: ' . print_r($_POST, true));
            $this->ajax_get_progress();
        });

        add_action('wp_ajax_get_scan_stats', function() {
            check_ajax_referer('bloom-scanner-nonce', 'nonce');
            error_log('get_scan_stats request data: ' . print_r($_POST, true));
            $this->ajax_get_stats();
        });
...
public function enqueue_assets($hook) {
    ...
    error_log('Generated nonce: ' . $nonce); // Debug log
    ...
    error_log('AJAX scan request received'); // Debug log

    if (!check_ajax_referer('bloom-scanner-nonce', 'nonce', false)) {
        error_log('Nonce verification failed'); // Debug log
    ...
    try {
        if ($this->scanning) {
            error_log('Scan already in progress'); // Debug log
        ...
        error_log('Starting media library scan'); // Debug log
        ...
        error_log('Scan error: ' . $e->getMessage()); // Debug log
   ...
   public function ajax_get_progress() {
    error_log('Progress check started');
    try {
        wp_send_json_success([
    ...
    public function ajax_get_stats() {
        error_log('Stats request started');
        try {
            $stats = get_option('bloom_chunk_scanner_stats', [
                'total_scanned' => 0,
                'chunks_found' => 0,
            ...
            error_log('Stats request error: ' . $e->getMessage());
            wp_send_json_error($e->getMessage());
        }
    }
